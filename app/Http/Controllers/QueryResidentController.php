...
class QueryResidentController extends Controller
{
    ...
    public function getQuery(Request $r) {
        if ($r -> ajax()) {
            return json_encode(\DB::table('residents') ->select('residentPrimeID','imagePath','residentID', 
                                                                'firstName','lastName','middleName','suffix', 
                                                                'residents.status', 'contactNumber', 'gender', 'birthDate', 
                                                                'civilStatus','seniorCitizenID','disabilities', 'residentType')
    												-> where('residents.status','1')
                                                    -> where('lastName','like','%'.$r->input('lastName').'%')
                                                    -> where('firstName','like','%'.$r->input('firstName').'%')
                                                    -> where('middleName','like','%'.$r->input('middleName').'%')
                                                    -> where('suffix','=',$r->input('suffix'))
                                                    -> where('gender','like','%'.$r->input('gender').'%')
                                                    -> where('civilStatus','like','%'.$r->input('civilStatus').'%')
                                                    -> where('disabilities','=',$r->input('disabilities'))
                                                    -> where('residentType','like','%'.$r->input('residentType').'%')
                                                    -> get());
        }
        else {
            return view('errors.403');
